...
HP_BOUNDS = {
    "lr":              (5e-5, 5e-4),
    "c_puct":          (1.0, 3.0),
    "value_weight":    (1.0, 5.0),
    "policy_weight":   (1.0, 2.0),
    "dirichlet_alpha": (0.15, 0.35),
    "dirichlet_eps":   (0.15, 0.25),
    "step_penalty":    (0.005, 0.03),#(0.0, 0.02),
    "entropy_coeff":   (0.001, 0.003)#(0.001, 0.01),
}
...
def train_agent(agent: AgentState, dataset: list) -> Tuple[float, float, float]:
    ...
    for _ in range(TRAIN_EPOCHS):
        ...
        for start in range(0, N, BATCH_SIZE):
            ...
            perm_colors = torch.randperm(6, device=DEVICE)
            aug_obs = bo[:, :, perm_colors]

            bo = torch.cat([bo, aug_obs], dim=0)
            bp = torch.cat([bp, bp], dim=0)
            bv = torch.cat([bv, bv], dim=0)
...
class PBTSelfPlayPool:
    ...
    def collect_for_agent(
        ...
        self.sp_manager.update_configs(
            ...
            mcts_hyperparams=mcts_hp, color_augmentation=True
        )
